...
let bgSat = minmax(35, 75);
let bgBri = 45-bgSat/4;
let txtr;
let bg;
...
let cnvs;
let sizes = [minmax(15, 20), minmax(10, 20), minmax(10, 12)];
  ...
  txtr=createGraphics(width, height)
	...
	for(var i=0;i<width;i++){
		for(var j=0;j<height;j++){
				...
				txtr.set(i,j,color(130,noise(i/3,j/3,i*j/50)*minmax(0, 100)))
		}
	}
	txtr.updatePixels()
...
if(frameCount <= 120){
  ...
  x = floor(X[a] + 60 * cos(floor(minmaxdouble(0, 9))*(TAU/4)));
  y = floor(Y[a] + 60 * sin(floor(minmaxdouble(0, 9))*(TAU/4)));


  stroke(lineColors[floor(x)]);

if(!lines.find((el) => abs(el[0] - floor(X[a])) <= 5 && abs(el[1] - floor(Y[a])) <= 5 && abs(el[2] - x) <= 5 && abs(el[3] - y) <= 5) && !lines.find((el) => abs(el[0] - x) <= 5 && abs(el[1] - y) <= 5 && abs(el[2] - floor(X[a])) <= 5 && abs(el[3] - floor(Y[a])) <= 5)){
...
x = floor(X[X.length - 1] + 60 * cos(floor(minmaxdouble(0, 8))*(TAU/4)));
y = floor(Y[Y.length - 1] + 60 * sin(floor(minmaxdouble(0, 8))*(TAU/4)));



  if(!lines.find((el) => abs(el[0] - floor(X[X.length - 1])) <= 5 && abs(el[1] - floor(Y[Y.length - 1])) <= 5 && abs(el[2] - x) <= 5 && abs(el[3] - y)<= 5) && !lines.find((el) => abs(el[0] - x) <= 5 && abs(el[1] - y) <= 5 && abs(el[2] - floor(X[X.length - 1])) <= 5 && abs(el[3] - floor(Y[Y.length - 1]))<= 5)){
