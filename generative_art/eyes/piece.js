...
function setup() {
  ...
  switch (type){
    case 1:
      ...
      break;
      case 2:
        circles =  createGrid(canvasSize, canvasSize, 3, 3, canvasSize/3);
        break;
  }
...
let hue = cusrand(0, 360), hue2 = cusrand(0, 360);
  ...
  eye.radius = cusrand(canvasSize/15, canvasSize/10);
  eye.speed = 3;
  eye.destination = getRandomPoint(eye.radius);
  eye.current = createVector(0, 0);
  eye.style1 = {fill:"white", stroke:ballFill, fillStyle:"cross-hatch", hachureGap:3, fillWeight:1}
...
switch (type){
  case 1:
    bgCnvs.noStroke()
    bgCnvs.fill(HSLToHex(eyes[0].hue, 80, 90))
  bgCnvs.rect(0, 0, canvasSize/2, canvasSize/2)
  bgCnvs.fill(HSLToHex(eyes[1].hue, 80, 90))
  bgCnvs.rect(canvasSize/2, 0, canvasSize, canvasSize/2)
  bgCnvs.fill(HSLToHex(eyes[2].hue, 80, 90))
  bgCnvs.rect(0, canvasSize/2, canvasSize/2, canvasSize)
  bgCnvs.fill(HSLToHex(eyes[3].hue, 80, 90))
  bgCnvs.rect(canvasSize/2, canvasSize/2, canvasSize, canvasSize)

    break;

    case 2:

    let squareSize = canvasSize / 3;
    bgCnvs.noStroke()
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        bgCnvs.fill(HSLToHex(cusrand(0 ,360), 80, 90)); 
        bgCnvs.rect(i * squareSize, j * squareSize, squareSize, squareSize);
      }
    }
...
const eye = eyes[e];
    ...
    if (eye.destination && eye.current) {
      const dx = eye.destination.x - eye.current.x;
      const dy = eye.destination.y - eye.current.y;
      const distSquared = dx * dx + dy * dy;

      if (distSquared > eye.speed * eye.speed) {
        const angle = Math.atan2(dy, dx);
        eye.current.x += eye.speed * Math.cos(angle);
        eye.current.y += eye.speed * Math.sin(angle);
      } else {
  ...
  eye.e2r.ellipse(eye.e2.width/2+ eye.current.x, eye.e2.height/2+eye.current.y, eye.radius, eye.radius, {seed:e + SEED, fill: eye.style2.fill, stroke:  eye.style2.stroke, fillStyle: eye.style2.fillStyle})
  
  eye.e3.clear()
  eye.e3r.ellipse(eye.e3.width/2+eye.current.x, eye.e3.height/2+eye.current.y, eye.radius*1.5, eye.radius*1.5, {seed:e + SEED, fill: eye.style3.fill, stroke:  eye.style3.stroke, fillStyle:  eye.style3.fillStyle, strokeWidth:2})
  
  eye.e5.clear()
  eye.e5r.ellipse(eye.e5.width/2+eye.current.x, eye.e5.height/2+eye.current.y, eye.radius*2, eye.radius*2, {seed:e + SEED, fill: eye.style5.fill, stroke:  eye.style5.stroke, fillStyle:  eye.style5.fillStyle})
